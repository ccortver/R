...
# La transformación logarítmica reduce la asímetria y mejora la distribución, así se pueden visualizar los datos y mejorar su comparación con otros
data2 = log2(data)
...
# "Col" define los colores de las cajas del box plot, "main" es para el título del gráfico y "las" rota las etiquetas del eje X
# ¿Qué es un boxplot?
# Es un gráfico estadístico que te permite ver datos de una manera más visual al mostrar la distribución - usa medidas como rango, cuartiles, mediana u outliers
boxplot(data2, col=c("#c90076", "blue", "skyblue",
	"yellow", "orange", "red"),
...
# Si, porque las separa entre wild type y knockout
...
# Una tabla de vectores con los valores de expresión de los wild type y los knockout
wt <- data[,1:3]
ko <- data[,4:6]
class(wt)
...
# ¿Cuál es la media más alta? La media de KO es 37460.5, más alta que la 35375.53 de WT
...
# OJO que aquí usamos los datos SIN TRANSFORMAR. ¿Por qué? Porque se asume que los datos están siguiendo una distribución más o menos normal
# ¿Cuántas valores tiene cada muestra? 3 por valores por WT y otros 3 por KO
pvalue = NULL 
tstat = NULL 
...
# ¿Qué pasa si le ponemos con una transformación de -log10? Se invierte la escala - los valores se vuelven más grandes, lo que facilita la visualización
...
# ¿Puedes representarlo en el gráfico? Con abline se representan
diff.mean_cutoff = 2
pvalue_cutoff = 0.01
abline(v = diff.mean_cutoff, col = "skyblue", lwd = 3)
#abline(v = -diff.mean_cutoff, col = "red", lwd = 3)
abline(h = -log10(pvalue_cutoff), col = "lightgreen", lwd = 3)
...
# Ahora las combinamos. ¿Cuántos genes cumplen los dos criterios? 6 genes
...
# Por lo normal la sobreexpresión se asocia con valores positivos, en este caso son valores negativos por la decicisón que se ha hecho al principio del código
...
# Las 3 réplicas de WT y las 3 de KO son inversamente proporcional, si en WT está sobreexpresado en KO estará reprimido
